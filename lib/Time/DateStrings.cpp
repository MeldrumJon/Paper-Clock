...
static char bufferDay[dt_MAX_STRING_LEN+1];  // must be big enough for longest string and the terminating null
static char bufferMon[dt_MAX_STRING_LEN+1];  // must be big enough for longest string and the terminating null
...
{
    strcpy_P(bufferMon, (PGM_P)pgm_read_ptr(&(monthNames_P[month])));
    return bufferMon;
}
...
char* monthShortStr(uint8_t month)
{
   for (int i=0; i < dt_SHORT_STR_LEN; i++)      
      bufferMon[i] = pgm_read_byte(&(monthShortNames_P[i+ (month*dt_SHORT_STR_LEN)]));  
   bufferMon[dt_SHORT_STR_LEN] = 0;
   return bufferMon;
}

char* dayStr(uint8_t day) 
{
   strcpy_P(bufferDay, (PGM_P)pgm_read_ptr(&(dayNames_P[day])));
   return bufferDay;
}
...
char* dayShortStr(uint8_t day) 
{
   ...
   for (int i=0; i < dt_SHORT_STR_LEN; i++)      
      bufferDay[i] = pgm_read_byte(&(dayShortNames_P[index + i]));  
   bufferDay[dt_SHORT_STR_LEN] = 0; 
   return bufferDay;
}
