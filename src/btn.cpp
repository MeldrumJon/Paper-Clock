...
#define DEBOUNCE_TICKS TIMER_TICKS(50)
#define LONGPRESS_TICKS TIMER_TICKS(3000)
...
#define BTN_DMASK 0x08
#define INT1_EN_MASK 0x02
...
void btn_enable(void) {
	...
	EIMSK |= INT1_EN_MASK; // Set interrupt mask on INT1
	cli();
	return;
}
...
void btn_disable(void) {
	...
	EIMSK &= ~(INT1_EN_MASK); // Disable interrupt
	cli();
	return;
}
